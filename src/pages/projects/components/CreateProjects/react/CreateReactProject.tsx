...
function TerminalPanel({
    ...
    const [lines, setLines] = useState<OutputLine[]>([
        { text: `Creating React project: ${data.name}`, type: "info" },
        { text: `Location: ${projectsPath}/${data.name}`, type: "info" },
        { text: `Package Manager: ${data.packageManager}`, type: "info" },
        { text: `Template: ${TEMPLATES.find((t) => t.id === data.template)?.name}`, type: "info" },
        { text: "", type: "output" },
    ]);
    ...
    const shouldAddLine = (line: string): boolean => {
        ...
        if (normalized.startsWith("$") && normalized.includes("create-react-app")) return false;
    ...
    useEffect(() => {
        ...
        const unlistenPromise = listen("react-output", (event: any) => {
            ...
            if (payload.type === "started" && payload.pid) {
                ...
                payload.data.split("\n").forEach((line: string) => {
                    ...
                    if (trimmed && !trimmed.startsWith("$")) {
                        if (
                            ...
                            !trimmed.includes("───") &&
                            !trimmed.startsWith("[") &&
                            !trimmed.includes("create-react-app")
                        ) {
                ...
                addLine(`➜ ${data.packageManager} run start`, "info");
        ...
        const sendCommand = async () => {
            ...
            try {
                ...
                if (data.template === "typescript") {
                    template = "typescript";
                } else if (data.template === "cra-template-pwa") {
                    template = "cra-template-pwa";
                } else if (data.template === "cra-template" && data.customTemplate) {
                    template = data.customTemplate;
                } else if (data.template === "custom" && data.customTemplate) {
                    template = data.customTemplate;
                }
                ...
                addLine(`> Creating React project: ${data.name}`, "info");

                await invoke("create_react_project", {
                ...
                <div className="p-4 font-mono text-xs h-96 overflow-y-auto">
                    ...
                    {isInstalling && !error && !done && (
                        <div className="flex items-center gap-2 mt-2 text-zinc-400">
                            ...
                            <span>Setting up React project...</span>
                        </div>
                    )}
                    <div ref={bottomRef} />
                </div>
                ...
                <Button
                    onClick={() =>
                        onDone({
                            ...
                            description:
                                data.description ||
                                `React app with ${data.template === "typescript" ? "TypeScript" : "JavaScript"}`,
                            config: {
                                port: data.port,
                                host: data.host,
                                packageManager: data.packageManager,
                ...
                <div className="space-y-4">
                    <div className="space-y-1.5">
                        ...
                        <p className="text-[11px] text-muted-foreground">
                            Will be created at {projectsPath}/{formData.name || "my-react-app"}
                        </p>
