...
import { NodeVersion } from "@/pages/nodejs/NodeManagerPage.tsx";
...
export function ProjectAssignmentRow({
    ...
    project: { id: number; name: string; type: string; nodeVersion: string | null };
    versions: NodeVersion[];
    defaultVersion: string;
}) {
    const [selected, setSelected] = useState<string | null>(project.nodeVersion);
                        ...
                        <div className="p-1 space-y-0.5">
                            ...
                            {installed.map((v) => (
                                <button
                                    ...
                                    className={`w-full flex items-center gap-2 px-3 py-2 rounded-lg text-xs text-left hover:bg-muted transition-colors ${selected === v.major ? "bg-muted" : ""}`}
                                >
                                    <span
                                        className={`w-1.5 h-1.5 rounded-full ${versionBadgeColor(v.major).includes("amber") ? "bg-amber-500" : versionBadgeColor(v.major).includes("blue") ? "bg-blue-500" : "bg-emerald-500"}`}
                                    />
                                    Node.js {v.major} (v{v.full})
                                </button>
                            ))}
                        </div>
                    </div>
