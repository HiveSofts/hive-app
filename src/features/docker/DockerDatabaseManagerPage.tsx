...
import { ContainerInfo, CreateContainerResult, CreateDatabaseContainerRequest, DBPreset } from "./services/types";
import { DB_PRESETS, DB_CATEGORY_LABELS } from "./services/config/dbPresets";
...
function getPresetForImage(image: string): DBPreset | null {
    ...
    return DB_PRESETS.find(p => lower.includes(p.id) || lower.includes(p.label.toLowerCase())) ?? null;
}

function stateColors(state: string) {
    switch (state) {
        case "running": return { border: "border-emerald-500/20", dot: "bg-emerald-500 animate-pulse", badge: "bg-emerald-500/10 text-emerald-600 border-emerald-500/25" };
        case "exited":  return { border: "border-zinc-500/15",    dot: "bg-zinc-400",                  badge: "bg-zinc-500/10 text-zinc-500 border-zinc-500/20" };
        default:        return { border: "border-amber-500/15",   dot: "bg-amber-500",                 badge: "bg-amber-500/10 text-amber-600 border-amber-500/20" };
    }
}
...
function CreateDatabaseWizard({ onClose, onCreate }: WizardProps) {
    ...
    const handleCreate = async () => {
        ...
        try {
            ...
            setLogs(l => [...l, res.success ? "✓ Container created successfully" : `✗ ${res.error}`]);
            setResult(res);
            setStep("result");
        } finally {
            setCreating(false);
    ...
    const filteredPresets = categoryFilter === "all" ? DB_PRESETS : DB_PRESETS.filter(p => p.category === categoryFilter);

    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
            <div className="bg-background border rounded-2xl w-full max-w-xl shadow-2xl overflow-hidden flex flex-col" style={{ maxHeight: "90vh" }}>
                {/* Header */}
                <div className="flex items-center justify-between px-5 py-4 border-b shrink-0">
                    <div className="flex items-center gap-2.5">
                        ...
                        <div>
                            ...
                            {preset && <p className="text-[11px] text-muted-foreground">{preset.label}</p>}
                        </div>
                    </div>
                        ...
                        return (
                            <React.Fragment key={s.key}>
                                <div className={cn(
                                    ...
                                    active ? "text-blue-600" : done ? "text-emerald-600" : "text-muted-foreground"
                                )}>
                                    <span className={cn(
                                    ...
                                    >
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-center gap-2">
                                                ...
                                                <span className="text-[10px] text-muted-foreground border rounded px-1 font-mono">v{p.defaultVersion}</span>
                                                <span className={cn(
                                                    "text-[10px] px-1.5 py-px rounded-full border ml-auto",
                                                    p.category === "relational" ? "border-blue-500/30 text-blue-600 bg-blue-500/10" :
                                                    p.category === "cache" ? "border-red-500/30 text-red-600 bg-red-500/10" :
                                                    p.category === "nosql" ? "border-emerald-500/30 text-emerald-600 bg-emerald-500/10" :
                        ...
                        <div className="space-y-4">
                            <div className="grid grid-cols-2 gap-3">
                                <FieldRow label="Container name">
                                    <Input value={form.containerName} onChange={e => set("containerName", e.target.value)} className="h-8 text-xs font-mono" placeholder="hive-mysql" />
                                </FieldRow>
                                <FieldRow label="Version">
                                    <select value={form.version} onChange={e => set("version", e.target.value)} className="w-full h-8 text-xs border rounded-md bg-background px-2 font-mono">
                                        {preset.versions.map(v => <option key={v} value={v}>{v}</option>)}
                                    </select>
                                </FieldRow>
                            </div>

                            <FieldRow label="Host port" hint={`Default for ${preset.label}: ${preset.defaultPort}`}>
                                <Input type="number" value={form.hostPort} onChange={e => set("hostPort", e.target.value)} className="h-8 text-xs font-mono" />
                            </FieldRow>

                            {preset.hasDatabase && (
                                <FieldRow label="Database name">
                                    <Input value={form.databaseName} onChange={e => set("databaseName", e.target.value)} className="h-8 text-xs font-mono" />
                                </FieldRow>
                            )}

                            {preset.hasUser && (
                                <FieldRow label="Username">
                                    <Input value={form.username} onChange={e => set("username", e.target.value)} className="h-8 text-xs font-mono" />
                                </FieldRow>
                            )}
                            ...
                            {preset.hasPassword && (
                                <FieldRow label="User password">
                                    <div className="flex gap-2">
                                        ...
                                        <Button size="sm" variant="outline" className="h-8 text-xs shrink-0" onClick={() => set("password", generatePassword())}>
                                            Generate
                                        </Button>
                                    </div>
                                </FieldRow>
                            ...
                            {preset.hasRootPassword && (
                                <FieldRow label="Root password">
                                    <div className="flex gap-2">
                                        <PasswordInput value={form.rootPassword} onChange={v => set("rootPassword", v)} />
                                        <Button size="sm" variant="outline" className="h-8 text-xs shrink-0" onClick={() => set("rootPassword", generatePassword())}>
                                            Generate
                                        </Button>
                                    </div>
                                </FieldRow>
                    ...
                    {step === "advanced" && (
                        <div className="space-y-4">
                            ...
                            <FieldRow label="Memory limit" hint="e.g. 512m, 1g, 2g">
                                <Input value={form.memoryLimit} onChange={e => set("memoryLimit", e.target.value)} className="h-8 text-xs font-mono" placeholder="512m" />
                            </FieldRow>

                            <FieldRow label="Restart policy">
                                <select value={form.restartPolicy} onChange={e => set("restartPolicy", e.target.value)} className="w-full h-8 text-xs border rounded-md bg-background px-2">
                            ...
                            <FieldRow label="Custom volume name" hint="Leave empty to auto-generate">
                                <Input value={form.customVolume} onChange={e => set("customVolume", e.target.value)} className="h-8 text-xs font-mono" placeholder={`hive_${form.containerName}_data`} />
                            </FieldRow>
                            ...
                            <div className="p-3 rounded-xl border bg-muted/20">
                                ...
                                <p className="text-[11px] text-muted-foreground mt-1">Data persists across container restarts.</p>
                            </div>
                        </div>
                    )}
                        ...
                        <div className="space-y-4">
                            {creating && (
                                <div className="space-y-1.5 p-3 rounded-xl border bg-muted/20 font-mono text-[11px]">
                                    {logs.map((l, i) => (
                                        <div key={i} className={l.startsWith("✓") ? "text-emerald-600" : l.startsWith("✗") ? "text-red-500" : "text-muted-foreground"}>
                                            {l}
                                        </div>
                                    ))}
                            ...
                            {!creating && result.success && (
                                <>
                                    <div className="flex items-center gap-3 p-4 rounded-xl bg-emerald-500/10 border border-emerald-500/25">
                                        ...
                                        <div>
                                            <p className="text-sm font-semibold text-emerald-700">Container created!</p>
                                            <p className="text-[11px] text-muted-foreground">{result.container_name} is running on port {result.port}</p>
                                        </div>
                                    </div>

                                    <div className="space-y-2">
                                        <p className="text-[11px] font-medium text-muted-foreground uppercase tracking-wider">Connection details</p>
                                        {[
                                    ...
                                    {result.connection_string && (
                                        <div className="space-y-1.5">
                                            <p className="text-[11px] font-medium text-muted-foreground uppercase tracking-wider">Connection string</p>
                                            <div className="flex items-center gap-2 p-3 rounded-lg bg-zinc-950 border font-mono text-[11px]">
                                                <span className="flex-1 text-emerald-400 break-all">{result.connection_string}</span>
                                                <button onClick={() => copy(result.connection_string!)} className="p-1 rounded hover:bg-zinc-800 transition-colors shrink-0">
                                    ...
                                    {logs.length > 0 && (
                                        <div className="p-3 rounded-xl border bg-muted/10 font-mono text-[11px] space-y-1 max-h-32 overflow-y-auto">
                                            {logs.map((l, i) => (
                                                <div key={i} className={l.startsWith("✓") ? "text-emerald-600" : "text-muted-foreground"}>{l}</div>
                                            ))}
                                        </div>
                                    )}
                                </>
                            ...
                            {!creating && !result.success && (
                                <div className="flex items-start gap-3 p-4 rounded-xl bg-red-500/10 border border-red-500/25">
                                    ...
                                    <div>
                                        <p className="text-sm font-semibold text-red-600">Failed to create container</p>
                ...
                <div className="flex items-center justify-between px-5 py-4 border-t bg-muted/10 shrink-0">
                    <div>
                        {step !== "type" && step !== "result" && (
                            <Button size="sm" variant="ghost" className="gap-1.5 text-xs" onClick={() => setStep(step === "config" ? "type" : "config")}>
                    ...
                    <div className="flex gap-2">
                        {step === "result" ? (
                            ...
                            <>
                                <Button size="sm" variant="outline" className="text-xs" onClick={() => setStep("advanced")}>
                                    Advanced <ChevronRight className="w-3.5 h-3.5 ml-1" />
                                </Button>
                                <Button size="sm" className="gap-1.5 text-xs bg-blue-600 hover:bg-blue-700 text-white" onClick={handleCreate} disabled={creating || !form.containerName}>
...
function LogsModal({ containerName, onClose }: { containerName: string; onClose: () => void }) {
    ...
    const fetch = async () => {
        setLoading(true);
        try { const l = await dockerService.getContainerLogs(containerName, 300); setLogs(l); }
    ...
    const colorLine = (line: string) => {
        ...
        if (l.includes("ready") || l.includes("started") || l.includes("success")) return "text-emerald-400";
            ...
            >
                <div className="flex items-center justify-between px-4 py-3 border-b border-zinc-800 bg-zinc-900">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        />
                        <button onClick={fetch} className="p-1.5 rounded hover:bg-zinc-700 text-zinc-500 hover:text-zinc-300">
                ...
                <div className="flex-1 overflow-y-auto p-4 font-mono text-[11px] space-y-px">
                    {loading ? (
                        ...
                        <div key={i} className={`flex gap-3 hover:bg-zinc-900/60 px-1 rounded ${colorLine(line)}`}>
                            <span className="text-zinc-700 select-none w-7 text-right shrink-0">{i + 1}</span>
...
function ShellModal({ container, onClose }: { container: ContainerInfo; onClose: () => void }) {
    ...
    const onKey = (e: React.KeyboardEvent) => {
        ...
        if (e.key === "ArrowUp") { const i = Math.min(histIdx + 1, cmdHist.length - 1); setHistIdx(i); setInput(cmdHist[i] ?? ""); e.preventDefault(); }
        if (e.key === "ArrowDown") { const i = Math.max(histIdx - 1, -1); setHistIdx(i); setInput(i === -1 ? "" : (cmdHist[i] ?? "")); e.preventDefault(); }
    };
            ...
            >
                ...
                <div className="flex-1 overflow-y-auto p-4 font-mono text-[12px]">
                    <div className="text-zinc-600 mb-4 text-[11px]">
                        Connected to <span className="text-emerald-400">{container.name}</span> ({container.image}) — ↑↓ for history
                    </div>
                    {history.map((h, i) => (
                        <div key={i} className="mb-3">
                            ...
                            {h.out && <pre className={`pl-5 mt-0.5 text-[11px] whitespace-pre-wrap break-all ${h.err ? "text-red-400" : "text-zinc-400"}`}>{h.out}</pre>}
                        </div>
                    ))}
                    <div ref={endRef} />
                </div>
...
function DatabaseCard({ container, onStart, onStop, onRestart, onRemove, onLogs, onShell }: DBCardProps) {
    ...
    return (
        <div className={cn("rounded-2xl border bg-card p-4 flex flex-col gap-3 transition-shadow hover:shadow-md", colors.border)}>
            {/* Top row */}
            <div className="flex items-start justify-between">
                <div className="flex items-center gap-2.5">
                    <div
                        className="w-10 h-10 rounded-xl flex items-center justify-center text-xl shrink-0"
                        style={preset ? { backgroundColor: preset.color + "18", border: `1px solid ${preset.color}28` } : {}}
                    >
                        {preset?.icon ?? "🗄️"}
                    </div>
                    <div>
                        <div className="flex items-center gap-1.5">
                            <span className="font-semibold text-sm leading-tight">{container.name}</span>
                            <span className={cn("text-[10px] font-medium px-1.5 py-0.5 rounded-full border", colors.badge)}>
                                <span className={cn("inline-block w-1.5 h-1.5 rounded-full mr-1", colors.dot)} />
                                {container.state}
                            </span>
                        </div>
                        <span className="text-[11px] text-muted-foreground font-mono">{container.image}</span>
                    </div>
                </div>
            </div>
            ...
            <div className="flex gap-1">
                {isRunning ? (
                    <button onClick={() => act("stop", () => onStop(container.name))} disabled={!!busy} className="flex-1 flex items-center justify-center gap-1.5 py-1.5 rounded-lg border hover:bg-red-500/10 hover:text-red-500 hover:border-red-500/30 text-muted-foreground text-[11px] transition-colors disabled:opacity-40">
                        {busy === "stop" ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Square className="w-3.5 h-3.5" />}
                        Stop
                    </button>
                ) : (
                    <button onClick={() => act("start", () => onStart(container.name))} disabled={!!busy} className="flex-1 flex items-center justify-center gap-1.5 py-1.5 rounded-lg border hover:bg-emerald-500/10 hover:text-emerald-600 hover:border-emerald-500/30 text-muted-foreground text-[11px] transition-colors disabled:opacity-40">
                        {busy === "start" ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Play className="w-3.5 h-3.5" />}
                        Start
                    </button>
                )}
                <button onClick={() => act("restart", () => onRestart(container.name))} disabled={!!busy} className="flex-1 flex items-center justify-center gap-1.5 py-1.5 rounded-lg border hover:bg-amber-500/10 hover:text-amber-600 hover:border-amber-500/30 text-muted-foreground text-[11px] transition-colors disabled:opacity-40">
            ...
            <div className="flex gap-1">
                <button onClick={() => onLogs(container.name)} className="flex-1 flex items-center justify-center gap-1.5 py-1.5 rounded-lg border hover:bg-blue-500/10 hover:text-blue-600 hover:border-blue-500/30 text-muted-foreground text-[11px] transition-colors">
                    <Terminal className="w-3.5 h-3.5" />
                    Logs
                </button>
                {isRunning && (
                    <button onClick={() => onShell(container)} className="flex-1 flex items-center justify-center gap-1.5 py-1.5 rounded-lg border hover:bg-purple-500/10 hover:text-purple-600 hover:border-purple-500/30 text-muted-foreground text-[11px] transition-colors">
                        <Monitor className="w-3.5 h-3.5" />
                        Shell
                    </button>
                )}
                <button onClick={() => setShowRemove(r => !r)} className="flex items-center justify-center gap-1 py-1.5 px-2.5 rounded-lg border hover:bg-red-500/10 hover:text-red-500 hover:border-red-500/30 text-muted-foreground text-[11px] transition-colors">
            ...
            {showRemove && (
                <div className="p-3 rounded-xl bg-red-500/10 border border-red-500/20 space-y-2">
                    ...
                    <div className="flex gap-1.5">
                        <button
                            onClick={() => { setShowRemove(false); act("remove", () => onRemove(container.name, false)); }}
                            className="flex-1 text-[11px] py-1.5 rounded-lg bg-red-500/20 text-red-600 hover:bg-red-500/30 transition-colors"
                        >
                            Keep data
                        </button>
                        <button
                            onClick={() => { setShowRemove(false); act("remove", () => onRemove(container.name, true)); }}
                            className="flex-1 text-[11px] py-1.5 rounded-lg bg-red-500 text-white hover:bg-red-600 transition-colors"
                        >
                            Delete all
                        </button>
                    </div>
                    <button onClick={() => setShowRemove(false)} className="w-full text-[11px] text-muted-foreground">Cancel</button>
                </div>
            )}
...
export default function DockerDatabaseManagerPage() {
        ...
        return (
            <div className="flex flex-col items-center justify-center min-h-[60vh] p-8 text-center">
                ...
                <p className="text-sm text-muted-foreground mb-6 max-w-sm">
                    {!dockerInfo.installed ? "Docker is not installed." : "Docker daemon is not running."}
                </p>
    ...
    const dbCategories = Array.from(new Set(
        containers.map(c => getPresetForImage(c.image)?.category).filter(Boolean)
    ));
    ...
    return (
        <div className="min-h-screen p-6 max-w-7xl mx-auto space-y-6">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-[12px] text-muted-foreground">
                            {dbRunning} running · {containers.length - dbRunning} stopped · Docker {dockerInfo.version}
                        </p>
                    </div>
                </div>
                <div className="flex gap-2">
                    ...
                    <Button size="sm" className="gap-1.5 text-xs h-8 bg-blue-600 hover:bg-blue-700 text-white" onClick={() => setShowWizard(true)}>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {[
                    { icon: <Activity className="w-4 h-4 text-emerald-500" />, label: "Running", value: dbRunning, color: "border-emerald-500/20 bg-emerald-500/5" },
                    { icon: <Database className="w-4 h-4 text-blue-500" />, label: "Total", value: containers.length, color: "border-blue-500/20 bg-blue-500/5" },
                    { icon: <HardDrive className="w-4 h-4 text-purple-500" />, label: "Volumes", value: containers.length, color: "border-purple-500/20 bg-purple-500/5" },
                    { icon: <Layers className="w-4 h-4 text-amber-500" />, label: "Compose", value: dockerInfo.compose_available ? "Ready" : "N/A", color: "border-amber-500/20 bg-amber-500/5" },
                ].map((s, i) => (
            ...
            <div className="flex items-center gap-3">
                <div className="relative flex-1 max-w-xs">
                    ...
                    <Input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search containers..." className="pl-8 h-8 text-xs" />
                </div>
                {containers.length > 0 && (
                    <div className="flex gap-1.5">
            ...
            {filtered.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-24 rounded-2xl border-2 border-dashed">
                    ...
                    <p className="text-sm font-medium text-muted-foreground mb-1">
                        {containers.length === 0 ? "No database containers" : "No matching containers"}
                    </p>
                    ...
                    {containers.length === 0 && (
                        <Button size="sm" className="gap-1.5 text-xs bg-blue-600 hover:bg-blue-700 text-white" onClick={() => setShowWizard(true)}>
                            <Plus className="w-3.5 h-3.5" />
