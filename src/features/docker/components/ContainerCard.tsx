import { Activity, Copy, ExternalLink, Loader2, Play, RotateCcw, Square, Terminal, Trash2 } from "lucide-react";
...
function getStateColor(state: string) {
    switch (state) {
        case "running": return { dot: "bg-emerald-500", badge: "text-emerald-500 border-emerald-500/30", border: "border-emerald-500/20" };
        case "exited": return { dot: "bg-zinc-400", badge: "text-zinc-400 border-zinc-500/30", border: "border-zinc-500/20" };
        default: return { dot: "bg-amber-500", badge: "text-amber-500 border-amber-500/30", border: "border-amber-500/20" };
    }
}

export function ContainerCard({ container, onStart, onStop, onRestart, onRemove, onViewLogs }: Props) {
    ...
    return (
        <div className={`rounded-xl border bg-card p-4 transition-all hover:shadow-md ${colors.border}`}>
            <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-2.5">
                    <div
                        className="w-9 h-9 rounded-xl flex items-center justify-center text-lg shrink-0"
                        style={preset ? { backgroundColor: preset.color + "20", border: `1px solid ${preset.color}30` } : {}}
                    >
                    ...
                    <div>
                        <div className="flex items-center gap-1.5">
                            ...
                            <Badge variant="outline" className={`text-[9px] px-1.5 py-0 ${colors.badge}`}>
                                <span className={`w-1.5 h-1.5 rounded-full ${colors.dot} mr-1 inline-block ${isRunning ? "animate-pulse" : ""}`} />
                                {container.state}
                            </Badge>
                        </div>
            ...
            {port && (
                <div className="flex items-center gap-1.5 mt-2 p-2 rounded-lg bg-muted/30 border">
                    ...
                    <button onClick={copyPort} className="ml-auto p-0.5 rounded hover:bg-muted transition-colors">
                        {copied ? <Activity className="w-3 h-3 text-emerald-500" /> : <Copy className="w-3 h-3 text-muted-foreground" />}
                    </button>
                </div>
            )}
            ...
            {showRemoveConfirm && (
                <div className="mt-3 p-3 rounded-xl bg-red-500/10 border border-red-500/20 space-y-2">
                    ...
                    <p className="text-[11px] text-muted-foreground">This will delete the container. Volume data can be preserved.</p>
                    <div className="flex gap-2">
                        <button
                            onClick={() => { setShowRemoveConfirm(false); action("remove", () => onRemove(container.name, false)); }}
                        ...
                        <button
                            onClick={() => { setShowRemoveConfirm(false); action("remove", () => onRemove(container.name, true)); }}
                            className="flex-1 text-[11px] py-1.5 rounded-lg bg-red-500 hover:bg-red-600 text-white font-medium transition-colors"
                        >
                            Remove + Delete data
                        </button>
                    </div>
                    <button onClick={() => setShowRemoveConfirm(false)} className="w-full text-[11px] py-1 text-muted-foreground hover:text-foreground transition-colors">
                        Cancel
                    </button>
                </div>
            )}
