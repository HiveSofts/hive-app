...
export const detectDocker = (): Promise<DockerInfo> => invoke("detect_docker");

export const getDockerInstallUrl = (): Promise<string> => invoke("get_docker_install_url");
...
export const installDockerLinux = (onLog: (line: string) => void): Promise<string> => {
    ...
    return invoke<string>("install_docker_linux").finally(() => unlisten.then((fn) => fn()));
};

export const listContainers = (all = true): Promise<ContainerInfo[]> =>
    invoke("list_docker_containers", { all });

export const createDatabaseContainer = (
    req: CreateDatabaseContainerRequest
): Promise<CreateContainerResult> => invoke("create_database_container", { req });
...
export const listVolumes = (): Promise<
    Array<{ name: string; driver: string; mountpoint: string }>
...
export const executeSql = (
    ...
    invoke("execute_sql_in_container", {
        containerName,
        dbType,
        username,
        password,
...
export const listComposeProjects = (): Promise<
    Array<{ name: string; status: string; services: number }>
