...
import { 
    ChevronRight, Cpu, Search, RefreshCw, 
    CheckCircle2, Package, Trash2, Download,
    GitBranch, Zap, Globe
} from "lucide-react";
...
export default function RuntimeManagerPage() {
    ...
    const renderPanel = () => {
        switch (lang.id) {
            case "php": return <PhpPanel lang={lang} />;
            default: return (
            ...
            <div className="border-b border-white/10 bg-black/20 backdrop-blur-sm sticky top-0 z-10">
                <div className="px-6 py-4 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h1 className="text-base font-semibold tracking-tight">Runtime Manager</h1>
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <Button size="sm" variant="outline" className="h-7 text-xs gap-1.5 border-white/10 bg-white/5">
            ...
            <div className="flex flex-1">
                                ...
                                >
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-center gap-1.5">
                                            ...
                                            {l.installed && (
                                                <div className={cn("w-1.5 h-1.5 rounded-full shrink-0", isSelected ? c.text.replace("text-", "bg-") : "bg-green-400/60")} />
                                            )}
                                        </div>
                ...
                <div className="flex-1 overflow-auto">
                    <div className="p-6 space-y-6">
                        <div className={cn("rounded-2xl border p-5", colors.border, "bg-gradient-to-br from-white/5 to-transparent")}>
                            <div className="flex items-center justify-between">
                                <div className="flex items-center gap-4">
                                    <div className={cn("w-14 h-14 rounded-2xl border flex items-center justify-center text-3xl", colors.border, colors.bg)}>
                                        {lang.icon}
                                    </div>
                                    <div>
                                ...
                                <div className="flex items-center gap-2">
                                    <Select defaultValue={lang.currentVersion || lang.versions[0]?.version}>
                                        <SelectTrigger className={cn("w-36 h-8 text-xs border", colors.border, "bg-white/5")}>
                                            <SelectValue />
                                        </SelectTrigger>
                                        <SelectContent>
                                            {lang.versions.map((v) => (
                                    ...
                                    {lang.installed ? (
                                        ...
                                        <Button
                                            size="sm"
                                            className={cn("h-8 text-xs gap-1.5", colors.bg, colors.text, "border", colors.border, "hover:opacity-90")}
                                        >
                            ...
                            <div className="grid grid-cols-4 gap-3 mt-5 pt-4 border-t border-white/10">
                                {[
                                    { label: "Versions", value: lang.versions.length, icon: <GitBranch className="w-3.5 h-3.5" /> },
                                    { label: "Pkg Manager", value: lang.packageManager.split(" / ")[0], icon: <Package className="w-3.5 h-3.5" /> },
                                    { label: "Status", value: lang.installed ? "Active" : "Not installed", icon: <Zap className="w-3.5 h-3.5" /> },
                                    { label: "Platform", value: "Cross-platform", icon: <Globe className="w-3.5 h-3.5" /> },
                                ].map((stat) => (
                                    <div key={stat.label} className="flex items-center gap-2">
                                        <div className={cn("w-7 h-7 rounded-lg flex items-center justify-center shrink-0", colors.bg, colors.text)}>
                                            {stat.icon}
                                        </div>
                                        <div>
