...
export function NodeInfoPanel({ versions }: NodeInfoPanelProps) {
    ...
    const runTest = () => {
        ...
        setTimeout(() => {
            ...
            setOutput(`v${versionMap[selectedVer] ?? "20.18.1"}`);
        }, 1200);
    };

    const selectedVersion = versions.find((v) => v.major === selectedVer);
    const info = [
        { label: "Version", value: selectedVersion?.full ?? "20.18.1" },
        { label: "Binary", value: `~/.hive/node/${selectedVer}/bin/node` },
        {
            label: "npm",
            value: selectedVer === "20" ? "10.9.0" : selectedVer === "18" ? "10.8.0" : "10.9.0",
        },
