...
#[tauri::command]
pub async fn create_vue_project(
    app: AppHandle,
    project_path: String,
    name: String,
    ...
    tauri::async_runtime::spawn_blocking(move || {
        ...
        let _ = app.emit(
            ...
            serde_json::json!({
                "runId": run_id,
                "type": "info",
                "data": format!("Running: {} create vue@latest {} {}", package_manager, name, features.join(" "))
            }),
        );

        let mut child = cmd
            .spawn()
            .map_err(|e| format!("Failed to start Vue process: {}", e))?;
        ...
        let status = child
            .wait()
            .map_err(|e| format!("Failed waiting for Vue process: {}", e))?;
        ...
        if !status.success() {
            let err = error_buffer
                .lock()
                .map(|e| e.clone())
                .unwrap_or_else(|_| "Unknown Vue error.".to_string());

            let message = if err.trim().is_empty() {
                format!("Vue failed with exit code: {:?}", status.code())
            } else {
                format!("Vue failed:\n{}", err)
            };
