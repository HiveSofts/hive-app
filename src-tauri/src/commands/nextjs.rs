...
fn is_valid_package_manager(manager: &str) -> bool {
...
pub async fn install_package_manager(manager: String) -> Result<(), String> {
    ...
    if manager == "bun" {
        return Err(
            "Automatic Bun installation is disabled. Please install Bun manually from https://bun.sh"
                .to_string(),
        );
    }
            ...
            match child.try_wait() {
                ...
                Ok(None) => {
                    if start.elapsed() > timeout {
                        ...
                        let _ = child.wait();
                        return Err(format!(
                            "Installing {} timed out after 300 seconds.",
                            manager
                        ));
                    }
