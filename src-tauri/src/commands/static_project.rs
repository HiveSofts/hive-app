...
<body>
    ...
    {}
                ...
                if name.is_empty() { "My Static Site" } else { &name },
                if create_css { r#"<link rel="stylesheet" href="styles.css">"# } else { "" },
                if name.is_empty() { "My Static Site" } else { &name },
                if create_js { r#"<script src="script.js"></script>"# } else { "" }
            );
